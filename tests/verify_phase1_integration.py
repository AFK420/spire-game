...
# 55. Full System-Wide Audit & Regression Invariants (Suite 62)
print("\n--- [Check 55] Full System-Wide Audit & Regression Invariants (Phases 0-18) ---")
state_types_src = (ROOT / "src/shared/StateTypes.luau").read_text(encoding="utf-8")
network_svc_src = (ROOT / "src/server/services/NetworkService.luau").read_text(encoding="utf-8")
game_config_src = (ROOT / "src/shared/GameConfig.luau").read_text(encoding="utf-8")
ui_controller_src = (ROOT / "src/client/UIController.client.luau").read_text(encoding="utf-8")
status_svc_src = (ROOT / "src/server/services/StatusService.luau").read_text(encoding="utf-8")
...
persist_svc_src = (ROOT / "src/server/services/PersistenceService.luau").read_text(encoding="utf-8")

# Structural & Architecture Invariants
test("AutoRunTestsInStudio = false" in game_config_src, "GameConfig disables AutoRunTestsInStudio by default")
test("export type MapNodeView" in state_types_src, "StateTypes exports MapNodeView")
test("export type ActionResult" in state_types_src, "StateTypes exports ActionResult")
test("MapNodes: { MapNodeView }" in state_types_src, "StateTypes.RunSnapshot includes MapNodes view list")
test("ProfileRevision: number" in state_types_src, "StateTypes.PlayerProfile includes ProfileRevision")
test("NetworkService.ActionResultEvent" in network_svc_src, "NetworkService exports ActionResultEvent")
test("NetworkService.ReturnToLobbyEvent" in network_svc_src, "NetworkService exports ReturnToLobbyEvent")
test("function NetworkService.sendActionResult" in network_svc_src, "NetworkService implements sendActionResult helper")
test("function ClassService.getGimmickTracker" in class_svc_src, "ClassService implements getGimmickTracker")

# UI Layer Audit Invariants
test('local ActionResultEvent = getEvent("ActionResult")' in ui_controller_src, "UIController resolves ActionResultEvent")
test('local ReturnToLobbyEvent = getEvent("ReturnToLobby")' in ui_controller_src, "UIController resolves ReturnToLobbyEvent")
test("ActionResultEvent.OnClientEvent:Connect" in ui_controller_src, "UIController connects to ActionResultEvent")
test('victoryView.Name = "VictoryView"' in ui_controller_src, "UIController implements VictoryView")
test('defeatView.Name = "DefeatView"' in ui_controller_src, "UIController implements DefeatView")
test("RETURN TO LOBBY" in ui_controller_src, "UIController views provide Return to Lobby buttons")
test("snapshot.MapNodes" in ui_controller_src, "UIController renders authoritative snapshot.MapNodes")

# Suite 62 Invariant Assertions in TestRunner
test("--- [Suite 62] System-Wide Audit & Regression Invariants ---" in testrunner_src, "TestRunner includes Suite 62")
test("[Suite 62.1] StatusService removeStatus returns true" in testrunner_src, "Suite 62 tests StatusService.removeStatus return contract")
test("[Suite 62.1] Poison status cleared after removeStatus" in testrunner_src, "Suite 62 tests StatusService.removeStatus state cleanup")
test("[Suite 62.2] CombatService test isolation is active" in testrunner_src, "Suite 62 tests CombatService test isolation lifecycle")
test("[Suite 62.2] StatusService test isolation is active" in testrunner_src, "Suite 62 tests StatusService test isolation lifecycle")
test("[Suite 62.3] Class selection rejected when ClassLocked is true" in testrunner_src, "Suite 62 tests ClassLocked re-selection rejection")
test("[Suite 62.4a] StartRun rejects unlocked party members" in testrunner_src, "Suite 62 tests StartRun unlocked member rejection")
test("[Suite 62.4b] StartRun rejects non-party initiator" in testrunner_src, "Suite 62 tests StartRun non-party initiator rejection")
test("[Suite 62.4c] StartRun rejects empty party" in testrunner_src, "Suite 62 tests StartRun empty party rejection")
test("[Suite 62.5] Added 4 party members up to cap" in testrunner_src, "Suite 62 tests 4-player party cap enforcement")
test("[Suite 62.5] 5th player join rejected by party cap" in testrunner_src, "Suite 62 tests 5th player rejection at cap")
test("[Suite 62.5] New player join rejected when run phase is not Lobby" in testrunner_src, "Suite 62 tests mid-run join rejection")
test("[Suite 62.6] 2/4 split votes (1 vs 1) keeps voting open" in testrunner_src, "Suite 62 tests map voting split-vote pending state")
test("[Suite 62.6] 2/4 votes for Node A does NOT trigger travel" in testrunner_src, "Suite 62 tests map voting strict majority requirement")
test("[Suite 62.6] 3/4 votes achieves strict majority and travels to ActiveRoom" in testrunner_src, "Suite 62 tests map voting strict majority consensus")
test("[Suite 62.7] RunSnapshot contains populated MapNodes" in testrunner_src, "Suite 62 tests RunSnapshot MapNodes projection")
test("[Suite 62.8a] Enemy Buff intent increased AttackPower by 7" in testrunner_src, "Suite 62 tests enemy Buff intent execution")
test("[Suite 62.8b] Enemy Debuff intent applied 5 Poison stacks to player" in testrunner_src, "Suite 62 tests enemy Debuff intent execution")
test("[Suite 62.8c] Poison status ticked and damaged player at turn start" in testrunner_src, "Suite 62 tests player turn-start status ticking")
test("[Suite 62.8d] Dead enemy did not attack player" in testrunner_src, "Suite 62 tests dead enemy loop skip invariant")
test("[Suite 62.9] AetherMage Cryomancer turn-end converted 2 unspent energy to 8 Shield" in testrunner_src, "Suite 62 tests Cryomancer turn-end gimmick")
test("[Suite 62.10] Necrobinder Reaper kill gimmick restored 6 HP" in testrunner_src, "Suite 62 tests Reaper kill gimmick HP restoration")
test("[Suite 62.10] Necrobinder Reaper gained 2 SoulTokens on kill" in testrunner_src, "Suite 62 tests Reaper kill gimmick token generation")
test("[Suite 62.10] GremlinHorn triggered BonusEnergy on kill" in testrunner_src, "Suite 62 tests GremlinHorn OnKill energy bonus")
test("[Suite 62.11] DamagePipeline correctly flagged IsCritical" in testrunner_src, "Suite 62 tests DamagePipeline critical hit calculation")
test("[Suite 62.12] Effective card cost calculated with CostReduction" in testrunner_src, "Suite 62 tests StatResolver card CostReduction")
test("[Suite 62.13] Card play targeting nonexistent enemy rejected" in testrunner_src, "Suite 62 tests card play failure on invalid target")
test("[Suite 62.14] Claimed draft card added to permanent CardCollection" in testrunner_src, "Suite 62 tests permanent vs runtime draft collection separation")
test("[Suite 62.15] resetToLobby sets Phase to Lobby" in testrunner_src, "Suite 62 tests resetToLobby clean run reset")
test("[Suite 62.16] saveProfile fail-closed guard blocks saving failed profiles" in testrunner_src, "Suite 62 tests DataStore fail-closed guard")
test("[Suite 62.17] reconcile stripped unknown class" in testrunner_src, "Suite 62 tests DataStore unknown class sanitization")
test("[Suite 62.17] reconcile stripped unknown skill" in testrunner_src, "Suite 62 tests DataStore unknown skill sanitization")
test("[Suite 62.17] resolveUpdateConflict rejected stale local revision" in testrunner_src, "Suite 62 tests optimistic concurrency stale revision rejection")
test("[Suite 62.17] Monotonically incremented ProfileRevision to 4" in testrunner_src, "Suite 62 tests ProfileRevision monotonic incrementation")
test("[Suite 62.18] NetworkService.ActionResultEvent exists" in testrunner_src, "Suite 62 tests ActionResultEvent existence")
test("[Suite 62.18] NetworkService.ReturnToLobbyEvent exists" in testrunner_src, "Suite 62 tests ReturnToLobbyEvent existence")
test("[Suite 62.18] NetworkService.sendActionResult helper exists" in testrunner_src, "Suite 62 tests sendActionResult helper existence")

# 56. Complete Audit & Regression Proof (Suite 63)
print("\n--- [Check 56] Complete Audit & Regression Proof (Suite 63) ---")
test("--- [Suite 63] Complete Audit & Regression Invariant Verification ---" in testrunner_src, "TestRunner includes Suite 63")
test("[Suite 63.1] continueFromRewards strictly rejects ActiveRoom phase" in testrunner_src, "Suite 63 tests combat skip exploit rejection")
test("[Suite 63.2a] 1/1 vote immediately achieves strict majority and enters ActiveRoom" in testrunner_src, "Suite 63 tests 1/1 map voting majority")
test("[Suite 63.2b] 2/4 split votes (1 vs 1) keeps voting open" in testrunner_src, "Suite 63 tests 2/4 split voting pending")
test("[Suite 63.2b] 2/4 votes for n4A does NOT trigger travel" in testrunner_src, "Suite 63 tests 2/4 votes does not trigger travel")
test("[Suite 63.2b] 3/4 votes for n4A achieves strict majority" in testrunner_src, "Suite 63 tests 3/4 votes achieves strict majority")
test("[Suite 63.2c] 2/3 votes for n3A achieves strict majority" in testrunner_src, "Suite 63 tests 2/3 votes achieves strict majority")
test("[Suite 63.2d] 2/2 connected votes achieves majority with 1 disconnected member" in testrunner_src, "Suite 63 tests disconnected member voting handling")
test("[Suite 63.3a] Failed card play did NOT increment Shadowblade ComboCount" in testrunner_src, "Suite 63 tests zero combo increment on failed play")
test("[Suite 63.3b] BloodPriest HP was NOT consumed on failed card play" in testrunner_src, "Suite 63 tests zero HP consumption for BloodPriest on failed play")
test("[Suite 63.3c] Mechanist ScrapParts NOT incremented on failed card play" in testrunner_src, "Suite 63 tests zero scrap increment on failed play")
test("[Suite 63.4a] Necrobinder SwarmLord spawned 3 BoneServants from kills" in testrunner_src, "Suite 63 tests SwarmLord 3 servant spawn on kills")
test("[Suite 63.4a] SwarmLord 3 BoneServants dealt 24 turn-end damage" in testrunner_src, "Suite 63 tests SwarmLord turn-end damage")
test("[Suite 63.4b] Beastmaster Wolf dealt 6 turn-end damage" in testrunner_src, "Suite 63 tests Beastmaster wolf turn-end damage")
test("[Suite 63.4c] Artificer 2 turrets granted 8 shield" in testrunner_src, "Suite 63 tests Artificer turn-end shield")
test("[Suite 63.5a] CentennialPuzzle returned 3 BonusDraw on damage taken" in testrunner_src, "Suite 63 tests CentennialPuzzle draw consumer")
test("[Suite 63.5b] BronzeScales returned 3 BonusDamage thorns" in testrunner_src, "Suite 63 tests BronzeScales thorns bonus damage")
test("[Suite 63.5c] Orichalcum granted 6 BonusShield at 0 Shield" in testrunner_src, "Suite 63 tests Orichalcum shield bonus consumer")
test("[Suite 63.6] Assassin 3rd combo card IS guaranteed critical" in testrunner_src, "Suite 63 tests Assassin 3rd card guaranteed critical")
test("[Suite 63.7] HeroicStrike base cost is 1 Energy" in testrunner_src, "Suite 63 tests skill cost baseline")
test("[Suite 63.8] startRun rejected when player profile failed to load" in testrunner_src, "Suite 63 tests startRun rejected on profile load failure")

# 57. Hardening & Blocker Repair Verification (Suite 64 & Structural Invariants)
print("\n--- [Check 57] Hardening & Blocker Repair Verification (Suite 64) ---")
test("--- [Suite 64] Hardening & Blocker Repair Regression Suite ---" in testrunner_src, "TestRunner includes Suite 64")
test("[Suite 64.1a] StatusService.applyStatus applies Strength" in testrunner_src, "Suite 64 tests applyStatus applies Strength")
test("[Suite 64.1b] StatusService.removeStatus returns true on existing status" in testrunner_src, "Suite 64 tests removeStatus returns true on existing status")
test("[Suite 64.1c] StatusService.removeStatus returns false on non-existent status" in testrunner_src, "Suite 64 tests removeStatus returns false on non-existent status")
test("[Suite 64.2] continueFromRewards rejected while RoomPhase is ActiveRoom" in testrunner_src, "Suite 64 tests combat skip exploit rejection")
test("[Suite 64.3a] startRun rejected with nil initiator" in testrunner_src, "Suite 64 tests startRun rejects nil initiator")
test("[Suite 64.3b] startRun rejected with non-party initiator" in testrunner_src, "Suite 64 tests startRun rejects non-party initiator")
test("[Suite 64.3c] startRun rejected when player has not locked class" in testrunner_src, "Suite 64 tests startRun rejects unlocked class")
test("[Suite 64.3d] startRun rejected when initiator is disconnected" in testrunner_src, "Suite 64 tests startRun rejects disconnected initiator")
test("[Suite 64.3e] startRun succeeds with valid locked connected initiator" in testrunner_src, "Suite 64 tests startRun succeeds with valid initiator")
test("[Suite 64.4a] Initial class selection locks class" in testrunner_src, "Suite 64 tests initial class selection locks class")
test("[Suite 64.4b] Duplicate class selection rejected once locked" in testrunner_src, "Suite 64 tests duplicate class selection rejection")
test("[Suite 64.5a] Player %d added under 4-player cap" in testrunner_src, "Suite 64 tests party members up to 4-player cap")
test("[Suite 64.5b] 5th player rejected due to 4-player cap" in testrunner_src, "Suite 64 tests 5th player rejection at cap")
test("[Suite 64.6] Late join rejected when run phase is not Lobby" in testrunner_src, "Suite 64 tests late join rejected during active run")
test("[Suite 64.7a] 1/4 vote recorded; travel pending" in testrunner_src, "Suite 64 tests map vote 1/4 pending")
test("[Suite 64.7b] 2/4 vote recorded; travel pending" in testrunner_src, "Suite 64 tests map vote 2/4 pending")
test("[Suite 64.7c] 3/4 vote reaches majority; consensus travel triggered" in testrunner_src, "Suite 64 tests map vote 3/4 consensus travel")
test("[Suite 64.8a] Traveled node is NOT marked cleared prior to room resolution" in testrunner_src, "Suite 64 tests node not cleared on travel")
test("[Suite 64.8b] Next tier paths NOT available while current node is uncleared" in testrunner_src, "Suite 64 tests next tier paths unavailable while uncleared")
test("[Suite 64.8c] Node marked cleared via markNodeCleared" in testrunner_src, "Suite 64 tests markNodeCleared marks node cleared")
test("[Suite 64.8d] Outgoing paths available after node is cleared" in testrunner_src, "Suite 64 tests outgoing paths available after node is cleared")
test("[Suite 64.9a] applyStatus rejects unimplemented status 'Chill'" in testrunner_src, "Suite 64 tests applyStatus rejects unimplemented Chill")
test("[Suite 64.9b] applyStatus rejects unimplemented status 'Freeze'" in testrunner_src, "Suite 64 tests applyStatus rejects unimplemented Freeze")
test("[Suite 64.9c] applyStatus accepts implemented status 'Poison'" in testrunner_src, "Suite 64 tests applyStatus accepts implemented Poison")
test("[Suite 64.10a] First lethal hit flags _killTriggered" in testrunner_src, "Suite 64 tests lethal hit sets _killTriggered")
test("[Suite 64.10b] Second hit retains _killTriggered without redundant re-triggering" in testrunner_src, "Suite 64 tests second hit does not re-trigger kill")
...
test("[Suite 64.15b] resetCooldowns resets inventory skill cooldown to 0" in testrunner_src, "Suite 64 tests SkillService.resetCooldowns on inventory skills")
test("[Suite 64.16a] getProfile returns nil when LoadFailed" in testrunner_src, "Suite 64 tests getProfile returns nil on LoadFailed")
test("[Suite 64.16b] saveProfile rejected when LoadFailed" in testrunner_src, "Suite 64 tests saveProfile rejected on LoadFailed")
test("[Suite 64.16c] addAetherShards blocked on LoadFailed" in testrunner_src, "Suite 64 tests addAetherShards blocked on LoadFailed")
test("[Suite 64.16d] retryLoadProfile recovers profile once store succeeds" in testrunner_src, "Suite 64 tests retryLoadProfile recovers profile")
test("[Suite 64.17] ProfileRevision increments monotonically on save" in testrunner_src, "Suite 64 tests ProfileRevision increments monotonically")
test("[Suite 64.18a] MapNodeView exposes NodeId alias" in testrunner_src, "Suite 64 tests MapNodeView exposes NodeId alias")
test("[Suite 64.18b] MapNodeView exposes Connections alias" in testrunner_src, "Suite 64 tests MapNodeView exposes Connections alias")
test("[Suite 64.18c] MapNodeView exposes Reachable and Cleared aliases" in testrunner_src, "Suite 64 tests MapNodeView exposes Reachable and Cleared aliases")
test("[Suite 64.19a] resetToLobby returns Phase to Lobby" in testrunner_src, "Suite 64 tests resetToLobby returns Phase to Lobby")

# Check source structural invariants across repaired files
status_svc_src = (ROOT / "src/server/services/StatusService.luau").read_text(encoding="utf-8")
effect_resolver_src = (ROOT / "src/server/services/EffectResolver.luau").read_text(encoding="utf-8")
damage_pipeline_src = (ROOT / "src/server/services/DamagePipeline.luau").read_text(encoding="utf-8")
card_svc_src = (ROOT / "src/server/services/CardService.luau").read_text(encoding="utf-8")
combat_svc_src = (ROOT / "src/server/services/CombatService.luau").read_text(encoding="utf-8")
persistence_svc_src = (ROOT / "src/server/services/PersistenceService.luau").read_text(encoding="utf-8")
run_mgr_src = (ROOT / "src/server/services/RunManager.luau").read_text(encoding="utf-8")
...
test("if def.Implemented == false then" in status_svc_src, "StatusService rejects unimplemented statuses")
test("function StatusService.syncEntityFromProjection" in status_svc_src, "StatusService syncEntityFromProjection is standalone function")
test("function DamagePipeline.setCritRngHook" in damage_pipeline_src, "DamagePipeline implements setCritRngHook")
test("function CardService.toCardView(instance: StateTypes.CardInstance, playerState: StateTypes.PlayerState?)" in card_svc_src, "CardService.toCardView accepts playerState")
test("EffectResolver.isSupportedEffectType" in combat_svc_src, "CombatService pre-validates effect types")
test("function PersistenceService.retryLoadProfile" in persistence_svc_src, "PersistenceService implements retryLoadProfile")
test("function SkillService.resetCooldowns" in skill_svc_src, "SkillService implements resetCooldowns")
test("function DungeonMap.markNodeCleared" in dungeon_map_src, "DungeonMap implements markNodeCleared")
test("dmgResult.IsDefeatedOrDowned and not (enemy :: any)._killTriggered" in effect_resolver_src, "EffectResolver guards against duplicate kill triggers")
test("RelicService.resetCombatState" in run_mgr_src, "RunManager resets relic combat state on resetToLobby")
...
# 61. Suite 66 Regression Tests (Final Correction Pass)
print("\n--- [Check 61] Suite 66 Regression Tests (Final Correction Pass) ---")
testrunner_src = (ROOT / "src/server/services/TestRunner.luau").read_text(encoding="utf-8")
status_svc_src = (ROOT / "src/server/services/StatusService.luau").read_text(encoding="utf-8")
action_tx_src = (ROOT / "src/server/services/ActionTransaction.luau").read_text(encoding="utf-8")
net_svc_src = (ROOT / "src/server/services/NetworkService.luau").read_text(encoding="utf-8")
run_mgr_src = (ROOT / "src/server/services/RunManager.luau").read_text(encoding="utf-8")
combat_svc_src = (ROOT / "src/server/services/CombatService.luau").read_text(encoding="utf-8")
skill_svc_src = (ROOT / "src/server/services/SkillService.luau").read_text(encoding="utf-8")

test("--- [Suite 66]" in testrunner_src, "TestRunner includes Suite 66")
test("[Suite 66.1c] Existing Poison with Duration = -1 survives snapshot" in testrunner_src, "Suite 66 tests Poison Duration = -1 survives snapshot")
test("[Suite 66.1j] Restored Poison has Duration = -1" in testrunner_src, "Suite 66 tests Poison restored with Duration = -1")
test("[Suite 66.2d] Ignite with Duration = 3 survives rollback with Duration = 3" in testrunner_src, "Suite 66 tests Ignite Duration = 3 survives rollback")
test("[Suite 66.2e] Stacks restored exactly to 2" in testrunner_src, "Suite 66 tests stacks restored exactly")
test("[Suite 66.3] Status tick after rollback does not error" in testrunner_src, "Suite 66 tests status tick after rollback does not error")
test("[Suite 66.4b] addPlayer returns nil when real player profile load failed" in testrunner_src, "Suite 66 tests addPlayer returns nil on LoadFailed")
test("[Suite 66.4c] No PlayerState created for LoadFailed real player" in testrunner_src, "Suite 66 tests no PlayerState created on LoadFailed")
test("[Suite 66.5b] RequestStateSync produces safe snapshot for LoadFailed player" in testrunner_src, "Suite 66 tests safe snapshot for LoadFailed player")
test("[Suite 66.6c] addPlayer succeeds after profile load recovers" in testrunner_src, "Suite 66 tests addPlayer succeeds after profile load recovery")
test("[Suite 66.7b] Successful action published combat events to clients" in testrunner_src, "Suite 66 tests successful action publishes combat events")
test("[Suite 66.8b] Failed action published zero events to clients" in testrunner_src, "Suite 66 tests failed action publishes zero combat events")
test("[Suite 66.8c] Failed action restored exact CombatLog count" in testrunner_src, "Suite 66 tests failed action restores exact CombatLog")
test("[Suite 66.9a] CombatLog size capped at 25" in testrunner_src, "Suite 66 tests CombatLog capped at 25")
test("[Suite 66.9b] CombatLog filled to exactly 25 after 30 events" in testrunner_src, "Suite 66 tests CombatLog fills to 25 after 30 events")
test("[Suite 66.10a] EquippedItems is cleared" in testrunner_src, "Suite 66 tests EquippedItems cleared in resetToLobby")
test("[Suite 66.10b] EquipmentInventory is cleared" in testrunner_src, "Suite 66 tests EquipmentInventory cleared in resetToLobby")
test("[Suite 66.10c] EquippedSkills is cleared" in testrunner_src, "Suite 66 tests EquippedSkills cleared in resetToLobby")
test("[Suite 66.10d] PassivePoints reset to 0" in testrunner_src, "Suite 66 tests PassivePoints reset to 0 in resetToLobby")
test("[Suite 66.10k] SkillInventory has exactly 4 starting skills" in testrunner_src, "Suite 66 tests starting skills re-provisioned in resetToLobby")
test("[Suite 66.11b] Persistent AetherShards survived" in testrunner_src, "Suite 66 tests persistent AetherShards survived resetToLobby")
test("[Suite 66.12b] Persistent unlocked skill appears in runtime SkillInventory" in testrunner_src, "Suite 66 tests persistent skill rehydrated into runtime SkillInventory")
test("[Suite 66.12c] No duplicate runtime instance created for ShieldBash" in testrunner_src, "Suite 66 tests no duplicate runtime instance created")
test("[Suite 66.13b] Corrupted skill ID ignored in SkillInventory" in testrunner_src, "Suite 66 tests corrupted skill ID ignored")
test("[Suite 66.14a] Persistent unlocked skill can be equipped" in testrunner_src, "Suite 66 tests persistent skill can be equipped")
test("[Suite 66.14c] Rehydrated persistent skill can be used in combat" in testrunner_src, "Suite 66 tests persistent skill can be used in combat")

# Structural and Code Invariants for Final Correction Pass
test("RemainingDuration" not in status_svc_src, "StatusService eliminates RemainingDuration")
test("Duration = inst.Duration or -1" in status_svc_src, "StatusService snapshots and restores Duration")
test("function ActionTransaction.isTransactionActive" in action_tx_src, "ActionTransaction implements isTransactionActive")
test("function ActionTransaction.bufferEvent" in action_tx_src, "ActionTransaction implements bufferEvent")
test("function ActionTransaction.commitCombatTransaction" in action_tx_src, "ActionTransaction implements commitCombatTransaction")
test("function NetworkService.setCombatEventHook" in net_svc_src, "NetworkService implements setCombatEventHook")
test("ActionTransaction.commitCombatTransaction(txSnapshot, activeCombat)" in combat_svc_src, "CombatService commits combat transaction on card play")
test("ActionTransaction.commitCombatTransaction(txSnapshot, combatState)" in skill_svc_src, "SkillService commits combat transaction on skill use")
test("isRealPlayer and not PersistenceService.isProfileLoaded(player)" in run_mgr_src, "RunManager.addPlayer enforces fail-closed profile check for real players")
test("table.clear(member.EquippedItems)" in run_mgr_src, "RunManager.resetToLobby clears EquippedItems")
test("table.clear(member.EquipmentInventory)" in run_mgr_src, "RunManager.resetToLobby clears EquipmentInventory")
test("table.clear(member.EquippedSkills)" in run_mgr_src, "RunManager.resetToLobby clears EquippedSkills")
test("table.clear(member.SkillInventory)" in run_mgr_src, "RunManager.resetToLobby clears SkillInventory")
test("table.clear(member.UnlockedSkills)" in run_mgr_src, "RunManager.resetToLobby clears UnlockedSkills")
test("table.clear(member.UnlockedPassives)" in run_mgr_src, "RunManager.resetToLobby clears UnlockedPassives")
...
if compiler_bin.exists():
    ...
    for lf in luau_files:
        rel_p = lf.relative_to(ROOT)
        res = subprocess.run([str(compiler_bin), str(lf)], capture_output=True, text=True)
        if res.returncode != 0:
            compile_failures.append((str(rel_p), res.stderr.strip() or res.stdout.strip()))
    test(len(compile_failures) == 0, f"All {len(luau_files)} Luau files compile cleanly with luau-compile (failures: {len(compile_failures)})")
    if compile_failures:
        for f, err in compile_failures:
            print(f"    Compile error in {f}: {err}")
else:
    print("  [SKIP] luau-compile.exe not found at root.")
